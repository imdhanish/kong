...
local SchemaState = {}
SchemaState.__index = SchemaState


function SchemaState.load(db)
    ...
    for k, v in pairs(subsystem_state) do
      if #v > 0 then
        if not schema_state[k] then
          schema_state[k] = setmetatable({}, SchemaMigrations_mt)
        end
  ...
  return setmetatable(schema_state, SchemaState)
end


function SchemaState:is_up_to_date()
